...
import { ArticleTrendStatistics } from "@/interface/article/statistics"
import { Response } from "@/interface/common/response"
import { dateFormatter } from "@/utils/converter"
import { getArticleTrend } from "@/api/article/statistics"
import { ElMessage } from "element-plus"

type EChartsOption = echarts.EChartsOption
type EChartsType = echarts.EChartsType

const state = ref<Array<ArticleTrendStatistics>>([])
const view = ref()
//图标实例
...
onMounted(() => {
    ...
    getArticleTrend()
        .then((data: Response<Array<ArticleTrendStatistics>>) => {
            if (data.status === 200) {
        ...
        .then(() => {
            chartInstance.setOption({
                ...
                series: [
                    {
                        data: state.value.map((i) => i.articleCount),
                    },
                ],
            })
