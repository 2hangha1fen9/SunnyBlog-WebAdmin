    ...
    <el-card class="setting-card">
        <template #header>
            <div class="config-submit">
                <span>网站公告</span>
                <el-button :loading="loading" @click="setNotice">保存</el-button>
            </div>
        </template>
        <div id="vditor-notice"></div>
    </el-card>
</template>
...
onMounted(() => {
    //markdown编辑器配置
    vditor.value = new Vditor("vditor-notice", {
        preview: {
            markdown: {
                mark: true,
            },
...
function setNotice() {
    loading.value = true
    notice.value = vditor.value?.getValue()
    setConfig("notice", notice.value)
